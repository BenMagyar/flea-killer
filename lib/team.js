...
exports.get = function(leagueType, leagueId, teamId, callback) {
	...
	var url = 'http://www.fleaflicker.com/' + leagueType + '/leagues/' + leagueId + '/teams/' + teamId + '?statType=0&season=2014';
	...
	request(url, function(error, response, body) {
		...
		var team = {
			...
			points: $('#body-left .panel-body').labelSearch('dt', 'Points'),
			players: []
		};
			...
			$('#row_0_0_' + i).each(function() {
				...
				if ($(this).find('td:first-child .player').length > 0) {
					var player = {
						...
						totalPoints: $(this).find('td .fp').first().text(),
						avgPoints: $(this).find('td .fp').eq(1).text()
					};
